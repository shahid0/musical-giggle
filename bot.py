...
class Tomartod:
    ...
    def play_game_func(self, amount_pass):
        ...
        for i in range(amount_pass):
            ...
            stars = res.json().get("data").get("stars")
            self.log(f"{hijau}stars: {putih}{stars}")
            self.countdown(30)
            point = random.randint(self.game_low_point, self.game_high_point)
            data_claim = json.dumps(
                {"game_id": "59bcd12e-04e2-404c-a172-311a0084587d", "points": point,"stars": stars}
            )
            ...
            self.log(f"{hijau}success {biru}claim{hijau} game point : {putih}{point} {hijau}stars:{putih}{stars}")
    ...
    def get_balance(self, token, acc, user_name, wallet):
        ...
        while True:
            ...
            if self.play_game:
                ...
                self.log(f"{hijau}{data['play_passes']}!")
